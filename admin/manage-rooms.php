...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    // ✅ Get badge (use custom if provided, else use dropdown value)
    $badge = trim($_POST['badge_custom']);
    if (empty($badge)) {
    ...
    $room_folder = $base_upload_dir . $slug . '/';
    
    // ✅ Handle Features (one per line)
    $features = [];
    if (isset($_POST['features']) && !empty(trim($_POST['features']))) {
        $lines = explode("\n", trim($_POST['features']));
    ...
    if (!file_exists($room_folder)) {
        mkdir($room_folder, 0777, true);
    }
    ...
    if (isset($_FILES['room_images']) && !empty($_FILES['room_images']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            if ($files['error'][$i] === UPLOAD_ERR_OK) {
                ...
                $target = $room_folder . $filename;
                if (move_uploaded_file($files['tmp_name'][$i], $target)) {
                    $uploaded_images[] = 'uploads/rooms/' . $slug . '/' . $filename;
                }
            }
        }
    }
    ...
    // ✅ INSERT Query with ALL fields (including badge)
    $stmt = $conn->prepare("INSERT INTO rooms (name, price, max_guests, description, features, badge, image, images, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')");
    $stmt->bind_param("sdisssss", $name, $price, $max_guests, $desc, $features_json, $badge, $first_image, $images_json);
    
    if ($stmt->execute()) {
        $message = '<div class="alert-success">✅ Room added successfully with ' . count($uploaded_images) . ' images! Badge: ' . $badge . '</div>';
    } else {
...
if (isset($_GET['edit_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $edit_room['images'] = json_decode($edit_room['images'] ?? '[]', true) ?: [];
        $edit_room['features'] = json_decode($edit_room['features'] ?? '[]', true) ?: [];
        $edit_room['slug'] = createSlug($edit_room['name']);
    }
}

// ============================================================
// 3. UPDATE ROOM (UPDATE) - FIXED
// ============================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("SELECT image, images FROM rooms WHERE id = ?");
    ...
    if (isset($_FILES['new_images']) && !empty($_FILES['new_images']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            if ($files['error'][$i] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($files['tmp_name'][$i], $target)) {
                    $existing_images[] = 'uploads/rooms/' . $new_slug . '/' . $filename;
                }
            }
        }
    ...
    $stmt = $conn->prepare("UPDATE rooms SET name = ?, price = ?, max_guests = ?, description = ?, features = ?, badge = ?, image = ?, images = ? WHERE id = ?");
    $stmt->bind_param("sdisssssi", $name, $price, $max_guests, $desc, $features_json, $badge, $first_image, $images_json, $id);
    
    if ($stmt->execute()) {
        $message = '<div class="alert-success">✏️ Room updated successfully!</div>';
        $edit_room = null;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    $room_folder = $base_upload_dir . $slug . '/';
    if (file_exists($room_folder)) {
        deleteDir($room_folder);
    }
    $stmt = $conn->prepare("DELETE FROM rooms WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        $message = '<div class="alert-success">🗑️ Room and all images deleted successfully!</div>';
    } else {
        $message = '<div class="alert-danger">❌ Failed to delete room.</div>';
    }
}

// ============================================================
// 5. FETCH ALL ROOMS (LIST)
// ============================================================
$rooms = [];
...
<head>
    <meta charset="UTF-8">
    <title>Manage Rooms | Royal Estate Admin</title>
...
<div class="admin-container">
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-nav">
            ...
            <a href="manage-rooms.php" class="active"><i class="fas fa-bed"></i> Manage Rooms</a>
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-row">
                    ...
                    <input type="number" step="0.01" name="price" placeholder="Price per night (LKR)" required>
                    <input type="number" name="max_guests" placeholder="Max Guests" required>
                </div>
                
                <!-- ✅ Room Type / Badge Selection -->
                <div class="form-row">
                    ...
                    <input type="text" name="badge_custom" placeholder="Custom Type (e.g., 'Garden View')">
                    <span style="font-size:0.75rem; color:#888;">Select a type OR enter custom</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="features" rows="4" placeholder="Free WiFi&#10;Smart TV&#10;AC&#10;King Bed"></textarea>
                    <p class="help-text">📝 Enter each feature on a new line. These will appear with icons on the frontend cards.</p>
                </div>
        ...
        <div class="card">
            ...
            <div style="overflow-x: auto;">
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (empty($rooms)): ?>
                            <tr><td colspan="9" style="text-align:center; padding:20px;">No rooms found. Add your first room above!</td></tr>
                        <?php else: ?>
                            <?php foreach($rooms as $room): ?>
                            <tr>
                                <td data-label="Image">
                                    <?php 
                                    $img_path = !empty($room['images']) ? $room['images'][0] : (!empty($room['image']) ? $room['image'] : '');
                                    if (!empty($img_path) && file_exists('../' . $img_path)): ?>
                                        <img src="<?php echo $img_path; ?>" class="room-thumb" alt="Room">
                                    <?php else: ?>
                                        <div class="room-thumb" style="background:#eee; display:flex; align-items:center; justify-content:center;">📷</div>
                                    <?php endif; ?>
                                </td>
                                <td data-label="ID"><?php echo $room['id']; ?></td>
                                <td data-label="Name"><?php echo htmlspecialchars($room['name']); ?></td>
                                <td data-label="Price">LKR <?php echo number_format($room['price']); ?></td>
                                <td data-label="Max Guests"><?php echo $room['max_guests']; ?></td>
                                <td data-label="Badge/Type">
                                    <span style="background:#c5a263; color:white; padding:2px 10px; border-radius:12px; font-size:0.7rem;">
                                        <?php echo htmlspecialchars($room['badge'] ?? 'Premium'); ?>
                                    </span>
                                </td>
                                <td data-label="Features"><?php echo count($room['features']); ?> feature(s)</td>
                                <td data-label="Images"><?php echo count($room['images']); ?> image(s)</td>
                                <td data-label="Actions">
                                    <div class="action-buttons">
                                        <a href="?edit_id=<?php echo $room['id']; ?>" class="btn-edit"><i class="fas fa-edit"></i> Edit</a>
                                        <form method="POST" style="display: inline-block;" onsubmit="return confirm('Delete this room and all images?');">
                                            <input type="hidden" name="id" value="<?php echo $room['id']; ?>">
                                            <input type="hidden" name="slug" value="<?php echo $room['slug']; ?>">
...
<!-- ============================================================ -->
<!-- ✅ EDIT MODAL - WITH ROOM TYPE -->
<!-- ============================================================ -->
<div id="editModal" class="modal <?php echo ($edit_room) ? 'active' : ''; ?>">
    <div class="modal-content">
        <span class="modal-close" onclick="closeModal()">&times;</span>
        <h3 style="margin-bottom: 1rem;">✏️ Edit Room</h3>
        <?php if ($edit_room): ?>
        <form method="POST" enctype="multipart/form-data" id="editForm">
            <input type="hidden" name="id" value="<?php echo $edit_room['id']; ?>">
            <input type="hidden" name="old_slug" value="<?php echo $edit_room['slug']; ?>">
            <input type="hidden" name="existing_images" id="existingImages" value='<?php echo json_encode($edit_room['images']); ?>'>
            
            <div class="form-row">
                <input type="text" name="name" value="<?php echo htmlspecialchars($edit_room['name']); ?>" required>
                <input type="number" step="0.01" name="price" value="<?php echo $edit_room['price']; ?>" required>
                <input type="number" name="max_guests" value="<?php echo $edit_room['max_guests']; ?>" required>
            </div>
            
            <!-- ✅ Room Type Selection -->
            <div class="form-row">
                <select name="badge" required>
                    <option value="Premium" <?php echo ($edit_room['badge'] ?? 'Premium') == 'Premium' ? 'selected' : ''; ?>>🏷️ Premium</option>
                    <option value="Luxury" <?php echo ($edit_room['badge'] ?? '') == 'Luxury' ? 'selected' : ''; ?>>💎 Luxury</option>
                    <option value="Standard" <?php echo ($edit_room['badge'] ?? '') == 'Standard' ? 'selected' : ''; ?>>⭐ Standard</option>
                </select>
                <input type="text" name="badge_custom" value="<?php echo htmlspecialchars($edit_room['badge'] ?? ''); ?>" placeholder="Custom Type (optional)">
                <span style="font-size:0.75rem; color:#888;">Select a type OR enter custom</span>
            </div>
            ...
            <div class="form-group">
                <label>Amenities / Features (one per line)</label>
                <textarea name="features" rows="4"><?php echo implode("\n", $edit_room['features'] ?? []); ?></textarea>
            ...
            <div class="form-group">
                ...
                <div id="existingImagesPreview" class="image-list">
                    <?php foreach($edit_room['images'] ?? [] as $img): ?>
            ...
            <a href="manage-rooms.php" style="margin-left: 10px; color:#c5a263;">Cancel</a>
        </form>
        <?php endif; ?>
    </div>
</div>
